...
import static com.videocall.datingapp.Main.Application.getCurrentDateAndTime;
import static com.videocall.datingapp.Main.Application.maxCallTimeCounter;
...
import com.videocall.datingapp.sinchex.audio_mgr.AudioPlayer;
...
public class CallScreenActivity extends SinchBaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final_UUID = getIntent().getStringExtra("final_UUID");
        final_name = getIntent().getStringExtra("final_name");
        final_image_url = getIntent().getStringExtra("final_image_url");

        Log.e("call Activity : ", "Call id "+mCallId);

//        UserSettings("call_counter");
    }
...
//        finish();
    ...
    private void getCounterAndUpdate() {
        Log.e(TAG, "getCounterAndUpdate: "+ callTime+" _ "+maxCallTimeCounter);
        if (callTime > maxCallTimeCounter && status){
            status=false;

            if (final_UUID.equals("")){
                finish();
            }else
                UserSettings("call_counter");
        }else {
    ...
    private void UserSettings(String call_counter_field) {
            ...
            firebaseFirestore.collection("users")
                    ...
                    .addOnCompleteListener(new OnCompleteListener<DocumentSnapshot>() {
                        ...
                        public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                            String user_setting = task.getResult().getString(call_counter_field);

                            call_counter = Integer.parseInt(user_setting);
                            call_counter=call_counter+1;


                            Map<String, Object> mapUserFeeds1 = new HashMap<>();
                            mapUserFeeds1.put(call_counter_field, String.valueOf(call_counter));
                            firebaseFirestore.collection("users")
                                    .document(currentUser)
                                    .update(mapUserFeeds1);
                            ...
                            String randomUid= String.valueOf(UUID.randomUUID());

                            Map<String, Object> final_data = new HashMap<>();
                            final_data.put("final_UUID", final_UUID);
                            final_data.put("final_name", final_name);
                            final_data.put("final_image_url", final_image_url);
                            final_data.put("final_serial_number", "1");
                            final_data.put("final_count_time",String.valueOf(call_counter));

                            Map<String, Object> mapUserFeeds = new HashMap<>();
//                                mapUserFeeds.put(userString, String.valueOf(call_counter));
                            mapUserFeeds.putAll(final_data);

                            firebaseFirestore.collection("users")
                                        .document(currentUser).collection("calling_users").document(String.valueOf(random+randomUid)).set(mapUserFeeds);
